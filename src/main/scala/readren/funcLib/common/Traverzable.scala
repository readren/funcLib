...
import readren.funcLib.dataTypes.Transition;
import readren.funcLib.dataTypes.{Tree,Branch,Leaf};
  ...
  trait Traverzable[T[_]] extends Functor[T] with Foldable[T] { self =>
    ...
    def traverseS[S, A, B](ta: T[A])(f: A => Transition[S, B]): Transition[S, T[B]] = {
      type Aux[X] = Transition[S, X]
      traverse[Aux, A, B](ta)(f)(Monad.transitionMonad)
    }

    // Copiado del libro: listing 12.11.
    def mapAccum[S, A, B](ta: T[A], s: S)(f: (A, S) => (B, S)): (T[B], S) =
      traverseS(ta)((a: A) => (for {
        s1 <- Transition.getState[S]
        (b, s2) = f(a, s1)
        _ <- Transition.setState(s2)
      } yield b)).run(s)
